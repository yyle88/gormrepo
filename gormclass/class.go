...
// ModelCols is used for models that implement the Columns method to return associated columns (cls).
// ModelCols 用于实现 Columns 方法以返回关联列（cls）的模型。
type ModelCols[CLS any] interface {
	Columns() CLS
}

// ModelClass is used for models that implement the Columns method to return associated columns (cls) and the TableName method to return the table name.
