...
func TestRepo_Repo(t *testing.T) {
	...
	t.Run("demo2", func(t *testing.T) {
		require.NoError(t, db.Transaction(func(tx *gorm.DB) error {
			res, err := repo.Repo(tx).First(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
...
func TestRepo_Gorm(t *testing.T) {
	...
	t.Run("demo2", func(t *testing.T) {
		require.NoError(t, db.Transaction(func(tx *gorm.DB) error {
			var account Account
			require.NoError(t, repo.Gorm(tx).First(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
				return db.Where(cls.Username.Eq("demo-2-username"))
			}, &account).Error)
...
func TestRepo_Wrap(t *testing.T) {
	...
	ctx := context.Background()
	var account Account
	require.NoError(t, repo.Wrap(ctx, db).First(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
		return db.Where(cls.Username.Eq("demo-1-username"))
	}, &account).Error)
	require.Equal(t, "demo-1-nickname", account.Nickname)
}
