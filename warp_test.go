...
func TestGormRepo_Gorm(t *testing.T) {
	...
	var account Account
	require.NoError(t, repo.Repo(db).Gorm().First(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
...
func TestGormWrap_Repo(t *testing.T) {
	...
	res, err := repo.Gorm(db).Repo().First(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
		return db.Where(cls.Username.Eq("demo-1-username"))
	})
...
func TestGormRepo_Mold(t *testing.T) {
	...
	t.Run("case-2", func(t *testing.T) {
		require.NoError(t, db.Transaction(func(tx *gorm.DB) error {
			var nickname string
			require.NoError(t, repo.Repo(tx).Mold().Invoke(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
...
func TestGormWrap_Mold(t *testing.T) {
	...
	t.Run("case-2", func(t *testing.T) {
		require.NoError(t, db.Transaction(func(tx *gorm.DB) error {
			var nickname string
			require.NoError(t, repo.Gorm(tx).Mold().Invoke(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
				return db.Where(cls.Username.Eq("demo-2-username")).Select(string(cls.Nickname)).First(&nickname)
			}).Error)
			require.Equal(t, "demo-2-nickname", nickname)
			return nil
...
func TestGormRepo_WithContext(t *testing.T) {
	...
	res, err := repo.Repo(db).WithContext(ctx).First(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
...
func TestGormWrap_WithContext(t *testing.T) {
	...
	var account Account
	require.NoError(t, repo.Gorm(db).WithContext(ctx).First(func(db *gorm.DB, cls *AccountColumns) *gorm.DB {
		value := ctx.Value(contextKeyType{})
		require.NotNil(t, value)
		rawValue, ok := value.(string)
		require.True(t, ok)
